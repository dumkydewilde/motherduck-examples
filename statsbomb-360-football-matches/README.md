...
```sql
ATTACH 'md:_share/statsbomb/80f66346-f45d-47c1-8b17-9fef083ba22b' AS sb (READ_ONLY);
FROM sb.marts.match_stats LIMIT 10;
```
...
| `DIVE_TITLE`, `SB_DATABASE` | env for `dive/scripts/deploy-dive.sh` | Dive title to create/update and the database its `sb` alias binds to. |

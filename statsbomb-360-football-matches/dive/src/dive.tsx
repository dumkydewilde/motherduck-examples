...
// The data this dive needs, so copies and local preview know to attach it: the
// public statsbomb share, mounted under the alias `sb` that the SQL below queries.
// Keep on one line. A deploy can override the binding via required_resources (see
// scripts/deploy-dive.sh); the live dive also sets it server-side.
export const REQUIRED_DATABASES = [{ type: "share", path: "md:_share/statsbomb/80f66346-f45d-47c1-8b17-9fef083ba22b", alias: "sb" }];
...
export default function StatsBomb360() {
  ...
  const matchesQ = useSQLQuery(`
    ...
    FROM "sb"."core"."matches" WHERE has_360
  ...
  const eventsQ = useSQLQuery(`
    ...
    FROM "sb"."marts"."replay_events" WHERE match_id = ${N(matchId)} ORDER BY idx
  `, { enabled: !!matchId });
  const dotsQ = useSQLQuery(`
    SELECT idx, dot, team, actor, keeper, x, y
    FROM "sb"."marts"."replay_dots" WHERE match_id = ${N(matchId)} ORDER BY idx, dot
  `, { enabled: !!matchId });
  const markersQ = useSQLQuery(`
    SELECT minute, second, period, kind, team, player, label, b_x, b_y
    FROM "sb"."marts"."replay_markers" WHERE match_id = ${N(matchId)} ORDER BY period, minute, second
  `, { enabled: !!matchId });
  const xgQ = useSQLQuery(`
    SELECT idx, shot_xg FROM "sb"."core"."stg_events"

...
#   SB_DATABASE        MotherDuck database bound to the dive's `sb` alias. Default statsbomb.
#   SB_RESOURCE_URL    Full resource URL for the `sb` alias (overrides SB_DATABASE) --
...
# The dive queries the database under the fixed alias `sb` (see src/dive.tsx);
# only the underlying md: database (or share) is configurable. A neutral alias
# avoids colliding with a database actually named `statsbomb` in the viewer's
# account (which would block attaching the share under the same name).
REQUIRED_RESOURCES="[{'url': '${RESOURCE_URL_SQL}', 'alias': 'sb'}]"
